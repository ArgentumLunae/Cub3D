...
void	find_wall_distance(int **map, t_ray *ray, uint32_t *side)
{
	int	hit;
...
t_column	*calculate_column(t_scene *scene, t_ray *ray, uint32_t *side)
{
	t_column	*column;
	...
	if (ray->wall_dist < EPSILON && ray->wall_dist > -EPSILON)
		column->lineheight = (long int)pow(10, 30);
	else
...
void	raycasting(t_scene *scene)
{
	...
	ft_memset(scene->walls->pixels, 0, scene->walls->width \
			* scene->walls->height * sizeof(uint32_t));
	pxl_x = 0;
	side = 0;
